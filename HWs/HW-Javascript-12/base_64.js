class Base64
{
    ...
    createCredentials = (userid, password) => 
    {
        if(userid.indexOf(':') != -1)
        {
            throw new Error("User-id must not contain single colon ':'");
        }
        var credentials = this.encode(userid.concat(':', password));
        return credentials;
    };

    divideCredentials = (credentials) =>
    {
        var string = this.decode(credentials);
